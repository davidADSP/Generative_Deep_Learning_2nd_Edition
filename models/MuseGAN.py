...
class MuseGAN:
    def __init__(
        ...
        self.weight_init = RandomNormal(mean=0.0, stddev=0.02)  # 'he_normal' #RandomNormal(mean=0., stddev=0.02)
    ...
    def train(
        self, x_train, batch_size, epochs, run_folder, print_every_n_batches=10, n_critic=5, using_generator=False
    ):
    ...
    def notes_to_midi(self, run_folder, output, filename=None):
        ...
        for score_num in range(len(output)):
            ...
            if filename is None:
                parts.write(
                    "midi", fp=os.path.join(run_folder, "samples/sample_{}_{}.midi".format(self.epoch, score_num))
                )
            else:
    ...
    def plot_model(self, run_folder):
        ...
        plot_model(
            self.critic, to_file=os.path.join(run_folder, "viz/critic.png"), show_shapes=True, show_layer_names=True
        )
        plot_model(
            self.generator,
