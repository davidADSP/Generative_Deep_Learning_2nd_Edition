...
class CycleGAN:
    ...
    def build_generator_resnet(self):
        ...
        def upsample(layer_input, filters):
            y = Conv2DTranspose(
                filters, kernel_size=(3, 3), strides=2, padding="same", kernel_initializer=self.weight_init
            )(layer_input)
            y = InstanceNormalization(axis=-1, center=False, scale=False)(y)
            y = Activation("relu")(y)
    ...
    def build_discriminator(self):
        def conv4(layer_input, filters, stride=2, norm=True):
            y = Conv2D(
                filters, kernel_size=(4, 4), strides=stride, padding="same", kernel_initializer=self.weight_init
            )(layer_input)
    ...
    def train_generators(self, imgs_A, imgs_B, valid):
        ...
        return self.combined.train_on_batch([imgs_A, imgs_B], [valid, valid, imgs_A, imgs_B, imgs_A, imgs_B])
